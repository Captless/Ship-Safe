...
RULES: list[Rule] = [
    Rule(
        ...
        description="Deployment or hosting configuration was detected in the project.",
        why_it_matters="Presence of deployment configuration makes the deployment security checks relevant to this project.",
        recommendation="",
        ai_fix_prompt="",
        is_presence_signal=True,
        evidence_signal="Deployment or hosting configuration detected",
        patterns=[r"(?i)(vercel\.json|netlify\.toml|dockerfile|docker-compose|\.github/workflows|cloudbuild|firebase\.json|render\.yaml|fly\.toml)"],
    ),
    Rule(
        rule_id="DEPLOY-101",
        title="Server entry point or hosting framework detected",
        category="deployment",
        severity=Severity.INFO,
        confidence=Confidence.MEDIUM,
        description="A server entry point or hosting framework was detected in the code.",
        why_it_matters="Presence of a server entry point makes the deployment security checks relevant to this project.",
        recommendation="",
        ai_fix_prompt="",
        is_presence_signal=True,
        evidence_signal="Server entry point or hosting framework found",
        patterns=[r"(?i)\b(gunicorn|uvicorn|waitress|passenger|pm2|serve|PORT|listen\s*\()"],
    ),
    Rule(
        rule_id="DEPLOY-001",
